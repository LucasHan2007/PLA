...
async def lifespan(_app: FastAPI):
    ...
    (settings.data_dir / "frameworks").mkdir(exist_ok=True)
    (settings.data_dir / "knowledge_graph").mkdir(exist_ok=True)
    (settings.data_dir / "profiles").mkdir(exist_ok=True)
    (settings.data_dir / "implementation").mkdir(exist_ok=True)
    yield
