...
const analysisTasks: AnalysisStepTask[] = [
  {
    ...
    summaryBridge:
      ...
      '「一眼说出 0 还是 9」对应输出离散类别标签（0 至 9）；「考试规则与及格线」即问题定义与测试集首位准确率（Top-1 Accuracy）验收标准；' +
      '「暂不涉及找题和部署」则对应专业版中系统边界限定为仅推理（Inference），不含数据采集与模型部署。',
    actions: [
  ...
  },
  {
    ...
    summaryBridge:
      '「练习册 / 正式测验」即专业版中的训练集与独立测试集（留出法划分）；「整张图猜一个数字」对应图像分类（Image Classification）单标签多分类；' +
      '「不框数字、不给像素涂色」说明本项目不是目标检测或语义分割；「标准答案」即真实标签（ground truth），「带答案学习」即监督学习（Supervised Learning）范式；' +
  ...
  },
  {
    ...
    summaryBridge:
      '「九九乘法表」即专业版中的基准数据集（Benchmark Dataset）——MNIST 手写数字库；「6 万练习题 + 1 万期末卷」对应 6 万训练样本与 1 万测试样本；' +
      '「小小黑白手写数字」即 28×28 单通道灰度图，展平后为 784 维像素向量；「标准答案 0–9」即十类标签空间；' +
      '「图书管理员核对缺页、贴错、某类偏少」则对应专业版中的样本–标签结构校验与类别均衡（class balance）等数据质量检查。',
    actions: [
  ...
  },
  {
    ...
    summaryBridge:
      '「找最像的 K 个师兄师姐投票」即 K 近邻（KNN）的最近邻检索与多数票决（Majority Voting）；「题库」即训练集，预测时临时比对体现惰性学习（lazy learning）特性；' +
      '「长得像」由欧氏距离（Euclidean Distance）在 784 维特征向量上度量；「把图变成一串数字」即图像展平为特征表示；' +
      '「K 太大 / 太小」的直觉，对应专业版中的偏差–方差权衡（bias–variance tradeoff）与超参数 K 的选取。',
    actions: [
  ...
  },
  {
    ...
    summaryBridge:
      '「盲测 / 从未见过的考卷」即独立测试集上的泛化评估；「答对几成」对应首位预测准确率（Top-1 Accuracy）主指标；' +
      '「95 分算优秀」即 MNIST 加 K 近邻基线通常不低于 95% 的经验阈值；「3 被认成 8 的错题表」即混淆矩阵（Confusion Matrix）所揭示的类别混淆模式；' +
      '「不能偷看期末卷改模型」则对应专业版禁止窥视测试集（peeking）——调参过程中不得反复查看测试集，以免评估失真。',
    actions: [
  ...
  },
  {
    ...
    summaryBridge:
      '「五个岗位」对应专业版五个模块：配置、加载数据、预处理、训练近邻、评估；「流水线 / 传送带」即端到端流水线（End-to-End Pipeline）；' +
      '「前站输出是后站输入」描述 原始像素 → 归一化特征矩阵 → 拟合后的近邻模型 → 预测结果与评估指标 的数据流；' +
      '「各司其职、不串岗」则对应单一职责原则（Single Responsibility）与模块间不可颠倒的依赖顺序。',
    actions: [
